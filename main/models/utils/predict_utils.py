...
def get_score(model,model_name, x_train, x_test, y_train, y_test):
    y_pred = model.predict(x_test)
    # 模型评估
    pred = (y_pred > 0.5).astype("int32")
    accuracy = accuracy_score(pred, y_test)
    print(f"{model_name} 测试集 accuracy:", accuracy)
    #  训练集
    y_pred_train = model.predict(x_train)
    # 模型评估
    pred = (y_pred_train > 0.5).astype("int32")
    accuracy = accuracy_score(pred, y_train)
    print(f"{model_name} 训练集 accuracy:", accuracy)
    #  训练集
    ...
    print(f"{model_name} 整体数据 accuracy:", accuracy)
